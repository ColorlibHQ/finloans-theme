...
function finloans_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'finloans' ),
        'next_string'     => esc_html__( '&raquo;', 'finloans' ),
        'before_output'   => '<div class="finloans-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'finloans_pagination_defaults', $defaults )
    );
