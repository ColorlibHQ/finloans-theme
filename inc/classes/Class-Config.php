...
// Final Class
final class Finloans {

	// Theme Version
	private $finloans_version = '1.0';
	...
	private function setup() {
		...
		$enqueu          = new finloans_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->finloans_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'finloans_content_width', 751 );
		...
		load_theme_textdomain( 'finloans', FINLOANS_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'finloans_site_logo_149x44', 149, 44, true );

		// Client logo size
		add_image_size( 'finloans_client_logo_229x100', 229, 100, true );

		// Single blog post image size
		add_image_size( 'finloans_single_blog_750x375', 750, 375, true );
		add_image_size( 'finloans_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu'	=> esc_html__( 'Primary Menu', 'finloans' ),
			'services'		=> esc_html__( 'Services', 'finloans' ),
			'useful-links'	=> esc_html__( 'Useful Links', 'finloans' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = FINLOANS_DIR_CSS_URI;
		$jsPath  = FINLOANS_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->finloans_version,
				),
				array(
					'handler' => 'finloans-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'finloans' ) ) {
	...
	private function customizer_init(){
		...
		// Instantiate finloans theme customizer
		$finloans_theme_customizer = new finloans_theme_customizer();
	}
	...
	public function epsilon_quickie(){
			...
			'links' => array(
				array(
					'link_to'   => 'finloans_options_panel',
	...
	public function set_elementor_flag(){
		update_option( 'finloans_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'finloans-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'finloans_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'finloans-elementor-default-style',  FINLOANS_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->finloans_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'finloans_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'finloans-elementor-notice', FINLOANS_DIR_JS_URI.'finloans-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'finloans-elementor-notice',
				'finloansElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'finloans-elementor-notice-nonce' ),
				)
			);
		}
