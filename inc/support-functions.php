...
function finloans_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'finloans' ) . '</span></li>';
...
// Blog categories
function finloans_featured_post_cat(){
...
// social media
if ( ! function_exists( 'finloans_social' ) ) {
	function finloans_social( $args = array() ) {
		...
		$url = finloans_opt( 'finloans_social_url' );
...
// header cart count
function finloans_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( FINLOANS_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'finloans' ); ?>">
...
function finloans_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'finloans_contact7_form_content', 10, 2 );