...
add_action( 'wp_enqueue_scripts', 'finloans_sl_enqueue_scripts' );
function finloans_sl_enqueue_scripts() {

	wp_enqueue_script( 'finloans-post-likes', FINLOANS_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'finloans-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'finloans' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_finloans_process_simple_like', 'finloans_process_simple_like' );
add_action( 'wp_ajax_finloans_process_simple_like', 'finloans_process_simple_like' );
function finloans_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'finloans' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'finloans' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'finloans' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'finloans' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'finloans' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'finloans' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'finloans_show_user_likes' );
add_action( 'edit_user_profile', 'finloans_show_user_likes' );
function finloans_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'finloans' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'finloans' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
